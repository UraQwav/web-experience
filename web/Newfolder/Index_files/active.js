...
var knowledge = [ 
	...
	[ "установка лабораторной работы номер 18", 
        "состоит", 
        "из 4 компонентов: диод. реостат. миллиамперметр. вольтметр. . Запуск установки осуществляется с помощью нажатия на кнопку включения. После запуска работы установки необходимо выждать некоторое время пока не нагреется диод, обычно этот процесс занимает не более пяти минут. После прогрева диода можно приступать к измерениям. В таблицу заносятся значения из вольтметра и Миллиамперметра, интервал записи измерений определяется по напряжению и не должен привышать четыре десятых вольта, сами измерения производят не менее 5 раз. изменение значений напряжения осуществляется с помощью изменения сопротивления реостатом, то есть регулирующей ручки р. а." ], 
